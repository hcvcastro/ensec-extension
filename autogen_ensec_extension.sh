...
sub read_args($)
{
    ...
    while (<$fh>) {
        ...
        if ( substr($_, 0, 1) eq "'" ) {
            print "Migrating options from the old autogen_ensec_extension.lastrun format, using:\n";
            my @opts;
            @opts = split(/'/);
            foreach my $opt (@opts) {
...
if ($src_path ne $build_path)
{
    ...
    system ("ln -sf $src_path/configure_ensec_extension.ac configure.ac");
}
system ("$aclocal $aclocal_flags") && die "Failed to run aclocal";
unlink ("configure");
#print "autoconf -I ${src_path}\n";
system ("autoconf -I ${src_path}") && die "Failed to run autoconf";
die "Failed to generate the configure_ensec_extension script" if (! -f "configure");

# Handle help arguments first, so we don't clobber autogen_ensec_extension.lastrun
for my $arg (@ARGV) {
    if ($arg =~ /^(--help|-h|-\?)$/) {
...
my $input = "autogen_ensec_extension.input";
my $lastrun = "autogen_ensec_extension.lastrun";
...
if (defined $ENV{NOCONFIGURE}) {
    ...
    # Save autogen_ensec_extension.lastrun only if we did get some arguments on the command-line
    if (! -f $input && @ARGV) {
        if (scalar(@cmdline_args) > 0) {
            # if there's already an autogen_ensec_extension.lastrun, make a backup first
            if (-e $lastrun) {
            ...
            my $fh;
            open ($fh, ">autogen_ensec_extension.lastrun") || die "Can't open autogen_ensec_extension.lastrun: $!";
            for my $arg (@cmdline_args) {
                print $fh "$arg\n";
            }
            close ($fh);
